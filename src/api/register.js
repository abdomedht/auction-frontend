...
>>>>>>> c79958e235edd9aed3176c79fd138325e6dca064
    ...
    if (error.response) { // Error response from the server
      const status = error.response.status;
      if (status === 400) {
        Alert.alert("Register Failed", "Email already exists ");
      } else if (status >= 500) { // Server-side error
        Alert.alert("Server Error", "Please try again later");
      } else {
    ...
    } else { // Other errors (e.g., setup or coding issues)
      console.error('Error:', error.message);
      Alert.alert("Unexpected Error", `An error occurred: ${error.message}`);
    }
  }
};
